...
def predict_istest(df, num_folds, stratified=False, debug=False):
    ...
    feats = [f for f in df.columns if f not in [
        'TARGET', 'SK_ID_CURR', 'SK_ID_BUREAU', 'SK_ID_PREV', 'index', 'istest']]

    for n_fold, (train_idx, valid_idx) in enumerate(folds.split(df[feats], df['istest'])):
        train_x, train_y = df[feats].iloc[
            train_idx], df['istest'].iloc[train_idx]
        valid_x, valid_y = df[feats].iloc[
            valid_idx], df['istest'].iloc[valid_idx]
    ...
    print('Full AUC score %.6f' % roc_auc_score(df['istest'], oof_preds))
    # Write submission file and plot feature importance
    if not debug:
        df['pred_istest'] = oof_preds
        df[['SK_ID_CURR', 'pred_istest']].to_csv(pred_file_name, index=False)
...
def main(debug=False):
    ...
    with timer("Run LightGBM with kfold"):
        oof_preds = predict_istest(
            df, num_folds=5, stratified=False, debug=debug)
    return oof_preds

if __name__ == "__main__":
    pred_file_name = "istest_preds.csv"
